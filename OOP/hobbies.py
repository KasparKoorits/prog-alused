...
def sort_by_most_hobbies(people_list: list) -> list:
    ...
    """
    msthobby = []
    for sname in people_list:
        msthobby.append(sname)
    hobbies_sorted = sorted(msthobby, key=lambda msthobby: (len(msthobby.hobbies) * -1, msthobby.first_name))
    return hobbies_sorted
...
def sort_by_least_hobbies(people_list: list) -> list:
    ...
    for sname in people_list:
        msthobby.append(sname)
    hobbies_sorted = sorted(msthobby, key=lambda msthobby: (len(msthobby.hobbies), msthobby.first_name))
    return hobbies_sorted
...
def sort_people_and_hobbies(people_list: list) -> list:
    ...
    for sname in people_list:
        msthobby.append(sname)
        sname.hobbies.sort()
    names_sorted = sorted(msthobby, key=lambda msthobby: msthobby.first_name)
    return names_sorted
